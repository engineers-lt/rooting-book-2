...
そんな夏、冒頭で触れた July Tech Festa 2017@<fn>{JTF2017} に参加して刺激を受け、めげずにがんばっていこうと思っていた自分を神は見離したりませんでした。フットワークのとても軽快な情シス担当と、自分が知る中で史上最強のインフラエンジニアがやって来てくれたのです。自分の見えていなかったところや届かなかったところにも目や手が回るようになり、想像を絶していた業務量も着実に残りが少なくなっていったのです。自分にとってこの年は地獄にも落ちたものの這い上がることもできた、奇跡の年でした。
...
これは、man rsync を実行したときに先頭に表示される説明です。自分はこのコマンドを様々な局面で利用しました。どういうケースで役に立ったのか、代表的な使い方をいくつか紹介します。

 * サーバー約90台のソースコードを集めて差分がないかをチェックする：authorized_keys の対象行先頭に @<code>{command="(実行したいコマンド)", ssh-rsa ...} のように記述をしておくことで、SSH接続したときにコマンドが稼働するようにして、回収サーバーに向けて rsync を実行。回収完了後に順次 diff コマンドをかけて回るシェルで運用。

 * Gitリポジトリに誤って push してしまったときに、とある時点まで戻す：@<code>{git checkout (戻したい時点のコミットハッシュ)} で戻す基準を準備しておいて、@<code>{rsync -av --delete --exclude ".git/" (基準ディレクトリ)/ (元に戻すディレクトリ) } で戻したい時点に合わせて、commit・push を行う。適宜、n オプションで dry-run を行うこともあり。

 * @<code>{--bwlimit オプション} で帯域を制御したうえで、負荷を上げないようにしつつデータベースサーバーからバックアップサーバーにベースバックアップファイルを転送し、転送完了したら削除。 結果の verify を行う rsync ならではの実施できることではないかと。

 * @<code>{/etc/rsyncd.conf} を用意してデーモンモードで使用する：設定しておくことで、@<code>{rsync [options] (転送元ディレクトリ) (転送先サーバ)::(転送先パス)} で通常よりも高速で負荷の軽い転送が実現できる
...
2019年2月からフリーランスとなって、はや1年半。世の中はコロナ渦というとても苦しい局面になってしまっています。絶え間なく流れる不景気ニュースなどを見てしまうたびに、何か自分ができることはないだろうか、と少し考えたりします。が、何をしようという決断までには到達できません。どうしても、何か効果が出ないとやっても仕方がないのかもしれない、と弱気になってしまうのが正直なところです。
...
2018年1月に副業・兼業の促進に関するガイドライン@<fn>{複業ガイド}が作成されて以降、複業をするという選択肢ができて、フリーランスになってからの自分もその機会を探してきました。それまで1つの会社に所属するだけであったところから、どのようなところを意識して複数の会社を仮想的に問題なく出入りしているのか。ポイントと思うところを、可能な範囲で記載します。

 * 副業は週に10時間を目処に：本業で週に40時間は取られてしまうので、副業で稼働できる時間は多くはありません。そのため、週10時間を基本条件に置きました。WFH（Work From Home）が広まった現状では通勤時間が減った分もう少し稼働可能になったかもしれませんが、今のところ一貫しています。聞き出すのが難しい部分もありますが、発注元の予算事情も聞いておくのが良いと思います。

 * 本業はできること基準・副業は挑戦基準：本業では求められる成果を上げる前提で、あわよくば届きそうな新たな領域に手を出すこともあります。一方、副業はリモートで平日夜間や休日という自分以外の人が稼働していない時間帯に作業をするのでリアルタイムな相談はほぼできず、そのために自身で課題を乗り越える意志が必要になると思います。細かな状況報告も重要で、副業とはいえ要求水準以上の成果を積み上げていかないと、その先の受注が途絶えることもあり得ます。

 * 本業は貸与PC・副業は自前PC：機密情報管理の観点からも、本業のPCは原則として貸与してもらうことにしています。できることなら必要なスペックは自身から遠慮なく提示しておくのが良いでしょう。貸与品は丁寧に扱い、契約終了時にはクリーンインストールを行って返却し、次の人が使いやすいようにすべきです。対して副業ではNDA（機密保持契約）を締結し内容を把握したうえで、自前のPCで作業を行っています。ここには、柔軟な環境構築力が求められます。また、できるだけローカルにはデータを持たず、クラウドストレージやプライベートリポジトリを利用することが、情報漏洩のリスクを下げ、受発注双方の関係に良い影響をもたらすと思います。ITという仕事をする以上、情報セキュリティの暗黙的なルールの感覚が必要不可欠ではないでしょうか。

===[/column]


//footnote[JTF2020][https://techfesta.connpass.com/event/175611/]
//footnote[JTF2017][https://2017.techfesta.jp/ 特に印象に残っているセッションは D20「情シスのひみつ。人とシステムをつなぐエンジニアリングとは」と A60「日本のエンジニアが今取り組むべきこと」]
//footnote[2月のメンテ][GlusterFS(3.3)からの脱却に成功した2018.2.7 ( https://wanderer.sogaoh.space/posts/2018/02/26/1/ ) ]
//footnote[5月のメンテ][pg_repackで...容量の危機を脱した2018.5.23 ( https://wanderer.sogaoh.space/posts/2018/05/28/1/ )]
//footnote[エンクロージャ][https://www.dell.com/ja-jp/work/shop/povw/storage-md1420 のような製品]
//footnote[複業ガイド][https://www.mhlw.go.jp/stf/seisakunitsuite/bunya/0000192188.html]